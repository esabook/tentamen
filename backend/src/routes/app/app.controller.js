...
import { allGrantedPermissions, defaultGrantedPermissions, permissions } from './starter-permissions.js';
  ...
  try {
    ...
    const operations = roles().map(role => ({
      updateOne: {
        ...
        update: { $setOnInsert: { _id: toSnakeCase(role.name), name: role.name, description: role.description } },
        upsert: true
      }
    }));
    ...
    const guruPermissions = await Permission.find({ _id: { $in: guruPermissionNames } }).select('_id');
    ...
    // 3. Ambil _id dari setiap permission yang didefinisikan di atas
    const adminPermissions = await Permission.find({ _id: { $in: adminPermissionNames } }).select('_id');
    ...
    res.status(500).json({ message: "Failed to initialize roles or assign permissions", error: error.message });
  }
};
  ...
  try {
    const operations = permissions().map(p => ({
      updateOne: {
        ...
        update: { $setOnInsert: { _id: toSnakeCase(p.name), name: p.name, category: p.category, description: p.description } },
        upsert: true
      }
    }));
